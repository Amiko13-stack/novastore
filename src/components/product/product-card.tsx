    ...
    <article className="group">
      <div className="relative overflow-hidden rounded-[26px] border border-black/[0.055] bg-[#e9e7e0] shadow-[0_14px_34px_rgba(17,17,17,.035)] transition duration-500 group-hover:-translate-y-1 group-hover:shadow-[0_22px_48px_rgba(17,17,17,.08)]">
        <Link href={`/products/${product.productId}`} className="block aspect-[0.88] overflow-hidden">
          <Image
            ...
            className="object-cover saturate-[0.94] transition duration-1000 ease-out group-hover:scale-[1.055]"
          />
        ...
        <WishlistButton productId={product.productId} initialSaved={saved} variant="card" />
        {product.featured ? (
          <span className="absolute left-3 top-3 rounded-full bg-zinc-950 px-3 py-1.5 text-[9px] font-semibold uppercase tracking-[0.16em] text-white">
            Featured
          </span>
        ) : null}
      </div>
      <div className="pt-4">
        <div className="flex items-center justify-between gap-3">
          <p className="text-[9px] font-semibold uppercase tracking-[0.21em] text-zinc-500">
        ...
        <Link href={`/products/${product.productId}`} className="mt-2 block text-[17px] font-medium tracking-[-0.035em] text-zinc-950 sm:text-lg">
          {product.name}
        </Link>
