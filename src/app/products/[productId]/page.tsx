...
export default async function ProductDetailsPage({ params }: ProductPageProps) {
  ...
  const [category, categoryProducts, wishlist] = await Promise.all([
  ...
  const productIsSaved = savedProductIds.includes(product.productId);
        ...
        <Container>
          <nav className="flex flex-wrap items-center gap-2 py-6 text-xs text-zinc-500 sm:py-8">
            ...
            {category ? (
              <>
                <span>/</span>
                <Link href={`/products?categoryId=${encodeURIComponent(category.categoryId)}`} className="transition hover:text-zinc-950">
                  {category.name}
                </Link>
              </>
            ) : null}
          ...
          <section className="grid gap-10 lg:grid-cols-[1.08fr_0.92fr] lg:gap-14 xl:gap-20">
            <div className="relative aspect-[0.92] overflow-hidden rounded-[30px] border border-black/[0.045] bg-[#ecece8] shadow-[0_18px_55px_rgba(17,17,17,.055)] lg:sticky lg:top-24 lg:self-start">
              <Image src={product.imageUrl} alt={product.name} fill priority sizes="(max-width: 1024px) 100vw, 55vw" className="object-cover saturate-[0.96]" />
              {product.featured ? <span className="absolute left-4 top-4 rounded-full border border-black/[0.05] bg-white/90 px-3 py-1.5 text-[9px] font-semibold uppercase tracking-[0.17em] shadow-sm backdrop-blur">Featured</span> : null}
            </div>

            <div className="py-2 lg:py-8 xl:py-14">
              <p className="text-[10px] font-semibold uppercase tracking-[0.24em] text-zinc-500">{category?.name ?? "Nova selection"}</p>
              <h1 className="mt-4 text-[clamp(2.8rem,5.5vw,5.5rem)] font-medium leading-[0.92] tracking-[-0.065em]">{product.name}</h1>
              ...
              <div className="mt-8 grid gap-3 sm:grid-cols-[1fr_auto]">
                ...
                <WishlistButton productId={product.productId} initialSaved={productIsSaved} variant="detail" />
              </div>

              <div className="mt-10 grid gap-3 sm:grid-cols-2">
                <div className="rounded-[22px] border border-zinc-300/70 bg-white/38 p-5">
                  <TruckIcon className="h-5 w-5" />
                  <p className="mt-4 text-sm font-semibold tracking-tight">Persistent shopping bag</p>
                  <p className="mt-1.5 text-xs leading-5 text-zinc-500">Quantities and bag state are stored through the DynamoDB server layer.</p>
                </div>
                <div className="rounded-[22px] border border-zinc-300/70 bg-white/38 p-5">
                  <ShieldIcon className="h-5 w-5" />
                  <p className="mt-4 text-sm font-semibold tracking-tight">Saved to your edit</p>
                  <p className="mt-1.5 text-xs leading-5 text-zinc-500">Wishlist state now persists separately for the current user.</p>
                </div>
              </div>
          ...
          {relatedProducts.length > 0 ? (
            <section className="mt-24 border-t border-zinc-300/60 pt-16 sm:mt-32 sm:pt-20">
              <div className="mb-10 flex items-end justify-between gap-4">
                ...
                {category ? <Link href={`/products?categoryId=${encodeURIComponent(category.categoryId)}`} className="hidden border-b border-zinc-300 pb-1 text-sm font-semibold transition hover:border-zinc-900 sm:block">View category →</Link> : null}
              </div>
              <ProductGrid products={relatedProducts} categoryNames={categoryNames} savedProductIds={savedProductIds} />
            </section>
          ) : null}
        </Container>
      </main>
